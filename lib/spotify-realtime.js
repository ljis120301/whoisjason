...
class SpotifyRealtimeService {
  constructor() {
    ...
    this.lastPlayedTrack = null; // Add property to store last played track
    this.isPolling = false;
    this.pollFrequency = 180000; // Poll every 3 minutes to reduce API load while keeping data reasonably fresh
    ...
    try {
      ...
      if (currentResponse.status === 200) {
        ...
        if (currentData && currentData.item) {
          ...
          // Store as last played track when we have a current track
          this.lastPlayedTrack = { ...newTrack };
        }
      } else if (currentResponse.status === 204) {
        // No track currently playing - use last played track as fallback
        newTrack = this.lastPlayedTrack;
      ...
      // If no current track and no last played track, get recently played
      if (!newTrack) {
        ...
        if (recentResponse.ok) {
          ...
          if (recentData.items && recentData.items.length > 0) {
            ...
            // Store as last played track for future fallbacks
            this.lastPlayedTrack = { ...newTrack };
          }
        }
      }
