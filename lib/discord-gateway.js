      ...
      case 0: // Dispatch
        if (t === 'READY') {
          ...
          if (d.user && d.user.id === process.env.DISCORD_USER_ID) {
            ...
            // Only update if this is a real presence change, not just reconnection
            if (this.lastPresenceUpdate !== newPresence.status || 
                !this.lastKnownPresence || 
                this.lastKnownPresence.status !== newPresence.status) {
