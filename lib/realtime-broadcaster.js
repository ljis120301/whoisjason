...
class RealtimeBroadcaster {
  constructor() {
    ...
    // Cache system - 5 minute TTL for dynamic data, prioritize showing cached data over loading
    this.cache = {
      github: { data: null, lastFetch: null, ttl: 8 * 60 * 60 * 1000 }, // 8 hours
      discord: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 }, // 5 minutes
      spotify: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 }, // 5 minutes - always return cached data
      steam: {
        onlineStatus: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 }, // 5 minutes
  ...
  async fetchAllData() {
    try {
      // Fetch Spotify data with 5-minute caching - always return cached data if available
      const spotifyStart = Date.now();
      let spotifyData = null;
      let spotifyError = null;

      try {
        // Always return cached data if we have it (prioritize showing data over loading)
        const cachedSpotify = this.getCachedData('spotify');

        // Check if we need to refresh (cache expired)
        if (!this.isCacheValid('spotify')) {
          ...
          if (spotifyRealtime.currentTrack) {
            ...
            this.updateCache('spotify', spotifyData);
          } else if (cachedSpotify) {
            // No current track but we have cached data - use it
            spotifyData = cachedSpotify;
          }
        } else {
          // Cache is valid, use cached data
          spotifyData = cachedSpotify;
          this.logPerformance('Spotify', !!spotifyData, 0, null, 'Data served from cache');
        }

        // Final fallback: always return cached data if fresh fetch fails
        if (!spotifyData && cachedSpotify) {
          spotifyData = cachedSpotify;
        }
      } catch (error) {
        spotifyError = error;
        // Even on error, return cached data if available
        const cachedSpotify = this.getCachedData('spotify');
        if (cachedSpotify) {
          spotifyData = cachedSpotify;
        }
  ...
  start() {
    ...
    this.broadcastInterval = setInterval(() => {
      this.fetchAllData();
    }, 180000); // Fetch every 3 minutes - reduces API load while keeping data fresh
  ...
  clearCache(service = null) {
    if (service) {
      ...
      this.cache = {
        ...
        discord: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 },
        spotify: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 },
        steam: {
          onlineStatus: { data: null, lastFetch: null, ttl: 5 * 60 * 1000 },
