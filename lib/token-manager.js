...
class TokenManager {
  ...
  async authenticateDiscordAutomatically() {
    console.log('[TOKEN-DEBUG] 🔐 Starting Discord automatic authentication...');
    ...
    try {
      ...
      console.log('[TOKEN-DEBUG]   - Bot token present:', !!botToken);
      console.log('[TOKEN-DEBUG]   - Bot token length:', botToken ? botToken.length : 0);
      console.log('[TOKEN-DEBUG]   - User ID present:', !!userId);
      console.log('[TOKEN-DEBUG]   - User ID value:', userId || 'undefined');

      if (!botToken || !userId) {
        console.error('[TOKEN-DEBUG] ✗ Missing Discord credentials');
        console.warn('Missing Discord credentials');
        return false;
      }

      console.log('[TOKEN-DEBUG] 🌐 Testing Discord API connectivity...');
      const apiUrl = `https://discord.com/api/v10/users/${userId}`;
      console.log('[TOKEN-DEBUG] - API URL:', apiUrl);
      console.log('[TOKEN-DEBUG] - Authorization header:', `Bot ${botToken.substring(0, 10)}...`);
      ...
      console.log('[TOKEN-DEBUG] - Response status text:', response.statusText);
      console.log('[TOKEN-DEBUG] - Response headers:', Object.fromEntries(response.headers.entries()));

      if (response.ok) {
        const userData = await response.json();
        console.log('[TOKEN-DEBUG] ✓ Discord API responded successfully');
        console.log('[TOKEN-DEBUG] - User data received:', {
          id: userData.id,
        ...
        console.error('[TOKEN-DEBUG] ✗ Discord API error response:');
        console.error('[TOKEN-DEBUG] - Status:', response.status);
        console.error('[TOKEN-DEBUG] - Status text:', response.statusText);
        console.error('[TOKEN-DEBUG] - Error body:', errorText);
        console.error('Discord bot token validation failed:', response.status);
        return false;
      }

    } catch (error) {
      console.error('[TOKEN-DEBUG] ❌ Exception during Discord authentication:');
      console.error('[TOKEN-DEBUG] - Error type:', error.constructor.name);
      console.error('[TOKEN-DEBUG] - Error message:', error.message);
      console.error('[TOKEN-DEBUG] - Error code:', error.code);
      console.error('[TOKEN-DEBUG] - Error stack:', error.stack);
      
      // Check for specific network errors
      if (error.code === 'ENOTFOUND') {
        console.error('[TOKEN-DEBUG] 🌐 DNS resolution failed - network connectivity issue');
      } else if (error.code === 'ECONNREFUSED') {
        console.error('[TOKEN-DEBUG] 🌐 Connection refused - Discord API unreachable');
      } else if (error.code === 'ETIMEDOUT') {
        console.error('[TOKEN-DEBUG] 🌐 Connection timeout - network latency issue');
      }
