...
class TokenManager {
  ...
  async refreshSpotifyToken() {
    try {
      ...
      let refreshToken = this.tokens.spotify?.refresh_token || process.env.SPOTIFY_REFRESH_TOKEN;
  ...
  async authenticateSpotifyAutomatically() {
    try {
      // If we already have a valid refresh token, we're good
      if (this.tokens.spotify?.refresh_token || process.env.SPOTIFY_REFRESH_TOKEN) {
